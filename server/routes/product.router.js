...
router.get('/getProducts',verifyToken,async (req,res,next)=>{
    try{
        var userId = req.decoded.data;
        var user = await User.findById(userId).populate('products');
        res.status(200).json({message:"Products List",products : user.products});
    }
    catch (err){
        next(err)
    }
...
.get('/getProduct/:productId',verifyToken,async (req,res,next)=>{
    ...
    try {
        ...
        res.status(200).json({message:"Product has been fetched",product:product});
    } catch (error) {
        next(error);
    }
})
.post('/createProduct', verifyToken,async (req, res, next) => {
  ...
  try {
      ...
      res.status(200).json({message :"updated the product",product:product});
  } catch (error) {
      next(error);
  }
